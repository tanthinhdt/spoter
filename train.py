...
def get_default_args():
    ...
    parser.add_argument(
        "--lr", type=float, default=0.001, help="Learning rate for the model training"
    )
    parser.add_argument(
        "--log_freq",
        type=int,
...
def train(args):
    ...
    for epoch in range(args.epochs):
        train_loss, _, _, train_acc = train_epoch(
            slrt_model, train_loader, cel_criterion, sgd_optimizer, device
        )
        ...
        if epoch % args.log_freq == 0:
            ...
            if val_loader:
                ...
                logging.info(
                    "[" + str(epoch + 1) + "] VALIDATION  acc: " + str(val_acc)
                )
    ...
    if eval_loader:
        for i in range(checkpoint_index):
            for checkpoint_id in ["t", "v"]:
                ...
                _, _, eval_acc = evaluate(
                    tested_model, eval_loader, device, print_stats=True
                )
